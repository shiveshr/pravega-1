...
public class TransactionalEventStreamWriterImpl<Type> implements TransactionalEventStreamWriter<Type> {
    ...
    private static class TransactionImpl<Type> implements Transaction<Type> {
        ...
        private final Pinger pinger;
        private final AtomicBoolean closed = new AtomicBoolean(false);
        private final TransactionalEventStreamWriterImpl<Type> writer;
        ...
        public void writeEvent(String routingKey, Type event) throws TxnFailedException {
            ...
            AtomicReference<DelegationTokenProvider> tokenProvider = new AtomicReference<>();

            SegmentTransaction<Type> transaction = inner.computeIfAbsent(s, x -> {
                if (tokenProvider.get() == null) {
                    tokenProvider.set(DelegationTokenProviderFactory.create(controller, s, AccessOperation.WRITE));
                }
                SegmentOutputStream out = writer.outputStreamFactory.createOutputStreamForTransaction(s, txId,
                        writer.config, tokenProvider.get());
                return new SegmentTransactionImpl<>(txId, out, writer.serializer);
            });
    ...
    public Transaction<Type> beginTxn() {
        ...
        return new TransactionImpl<Type>(writerId, txnId, new ConcurrentHashMap<>(), txnSegments.getStreamSegments(), controller, stream, pinger, this);
    }
    ...
    public Transaction<Type> getTxn(UUID txId) {
        ...
        return new TransactionImpl<Type>(writerId, txId, transactions, segments, controller, stream, pinger, this);
    }
