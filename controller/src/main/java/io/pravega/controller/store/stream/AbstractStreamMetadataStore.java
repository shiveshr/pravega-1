...
public abstract class AbstractStreamMetadataStore implements StreamMetadataStore {
    ...
    public CompletableFuture<VersionedTransactionData> getTransactionData(final String scopeName,
        ...
        return Futures.completeOn(getStream(scopeName, streamName, context).getActiveTransaction(txId)
                .thenApply(data -> {
    ...
    public CompletableFuture<VersionedMetadata<ActiveTxnRecord>> getTransaction(final String scopeName,
                                                                          ...
                                                                          final Executor executor) {
        int epoch = RecordHelper.getTransactionEpoch(txId);
        return Futures.completeOn(getStream(scopeName, streamName, context).getActiveTransaction(txId), executor);
    }
    ...
    protected ReaderGroup getReaderGroup(String scope, final String name, RGOperationContext context) {
        ...
        if (context != null) {
            ...
            readerGroup = newReaderGroup(scope, name);
        }
        return readerGroup;
    }
